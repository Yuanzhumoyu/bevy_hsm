...
- **灵活的转换触发器**:
  - **HSM**: 通过可组合的**条件系统** (`HsmOnEnterCondition`, `HsmOnExitCondition`) 自动触发转换。
  - **FSM**: 通过发送**事件** (`FsmOnTransition`) 来精确控制转换。
- **高级转换控制 (HSM)**:
...
- `StateConditions`: 用于注册和管理所有条件系统的资源。
- `StationaryStateMachine`: 一个标记组件，用于临时“暂停”一个状态机，使其不响应任何转换。
...
- `HsmOnState`: **HSM 的引擎**。这是一个特殊的组件，它的值 (`Enter`, `Update`, `Exit`) 决定了状态机当前所处的生命周期阶段，并通过 `on_insert` 钩子驱动所有逻辑。
- `StateTree`: 定义状态之间的父子层级关系。
- `HsmOnEnterCondition` / `HsmOnExitCondition`: 附加在状态实体上的组件，用于指定进入或退出该状态的条件。
...
- `FsmOnTransition`: **FSM 的引擎**。这是一个 Bevy 事件，发送此事件会触发 FSM 的状态转换。
- `FsmGraph`: 定义一个 FSM 中所有有效的转换路径。一个转换必须在图中被定义才能执行。
- `FsmOnEvent`: 一个 Trait，允许你使用自定义的任何类型（结构体、枚举、整数等）作为触发 FSM 转换的特定事件。
